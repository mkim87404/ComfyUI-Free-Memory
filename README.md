...
Secure & device-agnostic ComfyUI custom nodes for unloading a model or all models at any point in a workflow, followed by a robust set of VRAM & RAM clean up operations using the memory management utilities already present in ComfyUI. These nodes can also be used to persist & route any given data throughout the memory cleanup process, allowing the user to control the order of execution of nodes in the workflow. Forked from https://github.com/SeanScripts/ComfyUI-Unload-Model and inspired by https://github.com/ShmuelRonen/ComfyUI-FreeMemory
...
**Please note:** these nodes are dependent on the ComfyUI implementation of its `comfy.model_management.py` module, and may stop working in future ComfyUI updates. In particular, there have been some changes in the GGUF loader nodes that could cause the GGUF models to not unload (although this might've been fixed already).

## 🍃 Added Changes (Post-Fork)

1. Added 7 optional "passthrough" arguments that can be used to persist & route any data throughout this node's memory cleanup process e.g. latents, conditioning, images, masks, and other metadata.

    - While this passthrough of data may not be strictly necessary for **data persistence** (i.e. data variables are persisted as long as they are referenced by some later nodes elsewhere in the workflow), this explicit passthrough design can be very useful for routing and **controlling the execution order** of nodes in a more deterministic and predictable way, as well as tidying up & linearizing the node connections visually as your workflow gets bigger.
